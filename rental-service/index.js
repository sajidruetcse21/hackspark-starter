...
// --- Central API Rate Limiter (Proactive) ---
// The Central API strictly limits to 30 requests per minute per token.
// Violations incur a -20 point penalty. This interceptor spaces out requests
// by 2050ms (slightly over 2 seconds) to guarantee we never exceed the limit.
let nextAvailableTime = Date.now();
const MIN_INTERVAL = 2050; 

centralApi.interceptors.request.use(async (config) => {
  const now = Date.now();
  if (nextAvailableTime <= now) {
    nextAvailableTime = now + MIN_INTERVAL;
    return config;
  } else {
    const delay = nextAvailableTime - now;
    nextAvailableTime += MIN_INTERVAL;
...
app.get('/rentals/users/:id/top-categories', async (req, res) => {
  ...
  try {
    ...
    // --- 2. Fetch all rentals for the user (Pagination) ---
    while (true) {
      const centralRes = await centralApi.get('/api/data/rentals', {
      ...
      // If we received less than the limit, we've hit the last page
      if (data.length < 100) break;
      page++;
    }

    // Edge Case: User has zero rentals
    ...
    // --- 4. Fetch product categories in batches of 50 ---
    const productCategoryMap = {}; // Maps productId -> category
    ...
    for (let i = 0; i < productIds.length; i += 50) {
      ...
      // CRITICAL: Remember the Central API wraps batch results in .data.data
    ...
    // --- 5. Tally the categories based on ALL rentals ---
    // Note: We count the rentals, not just the unique products. 
    // If they rented the same camera 5 times, that's 5 points for ELECTRONICS.
    const counts = {};
    for (const rental of allRentals) {
      const category = productCategoryMap[rental.productId];
      if (category) {
        counts[category] = (counts[category] || 0) + 1;
      }
    }

    // --- 6. Sort and extract top K ---
    // Convert the counts object into an array of objects
    const categoryArray = Object.keys(counts).map(category => ({
      category,
      rentalCount: counts[category]
    }));

    // Sort descending by rentalCount, then slice the top K
    // (If K exceeds the distinct categories, .slice(0, K) safely handles it!)
    ...
    res.status(500).json({ error: "Internal server error fetching top categories" });
  }
});
